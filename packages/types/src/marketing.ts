// import { TFileAntd } from 'component-base/antd/UploadBase';
...
export enum COMBO_VOUCHER_TARGET_USER {
  BOTH = 'BOTH',
  NEW = 'NEW',
  CURRENT = 'CURRENT',
}
...
export type TAskerRank =
  | 'ASKER_RANK_NAME_MEMBER'
  | 'ASKER_RANK_NAME_SILVER'
  | 'ASKER_RANK_NAME_GOLD'
  | 'ASKER_RANK_NAME_PLATINUM';
...
export interface IComboVoucherItem
  extends Omit<
    IComboVoucherForm,
    'thumbnail' | 'image' | 'vouchers' | 'rangeDate'
  > {
  _id: string;
  createdAt: Date;
  startDate: Date;
  ...
  image: string;
  vouchers: Array<
    Omit<IVouchersForm, 'image' | 'brandInfo' | 'promotion' | 'applyFor'> & {
      _id?: string;
      image: string;
      brandInfo: {
        image: string;
